...
Agent.travelTo = function (goal_point) {
  (this.trip.current_point = this.getLatLng()),
    ...
    (this.trip.lat_dir = Math.sign(
      -(this.trip.current_point.lat - this.trip.goal_point.lat),
    )),
    (this.trip.lng_dir = Math.sign(
      -(this.trip.current_point.lng - this.trip.goal_point.lng),
    )),
    (this.trip.angle = bearing(
      L.A.pointToCoordinateArray(this.trip.current_point),
      L.A.pointToCoordinateArray(this.trip.goal_point),
    ));
  this.trip.slope = Math.abs(
    (this.trip.current_point.lat - this.trip.goal_point.lat) /
      (this.trip.current_point.lng - this.trip.goal_point.lng),
  );
  ...
  if (
    this.trip.path[0].new_place.type === "unanchored" ||
    this.trip.path[0].move_directly === true
  ) {
 ...
 */
Agent.setTravelToPlace = function (
  goal_lat_lng,
  goal_place,
  speed = 1,
  move_directly = false,
  ...
  if (
    start_place.type === "unanchored" ||
    goal_place.type === "unanchored" ||
    move_directly === true
  ) {
  ...
  let goal_layer =
    this.agentmap.units.getLayer(goal_place.id) ||
    this.agentmap.streets.getLayer(goal_place.id);
  ...
  if (goal_layer) {
    ...
    let goal_polygon = buffer(goal_layer.toGeoJSON(), 0.001);

    if (booleanPointInPolygon(goal_coords, goal_polygon)) {
      if (
        start_place.type === "unit" &&
        goal_place.type === "unit" &&
        start_place.id === goal_place.id
      ) {
      ...
      throw new Error(
        "The goal_lat_lng is not inside of the polygon of the goal_place!",
      );
    }
  } else {
...
Agent.setTravelAlongStreet = function (goal_lat_lng, goal_place, speed) {
  ...
  if (start_place.type === "street" && goal_place.type === "street") {
    ...
    (goal_street_id = goal_place.id),
      (goal_street_feature =
        this.agentmap.streets.getLayer(goal_street_id).feature),
      (goal_coords = L.A.pointToCoordinateArray(goal_lat_lng)),
      (goal_street_point = L.latLng(
        nearestPointOnLine(
          goal_street_feature,
          goal_coords,
  ...
  //to the intersection nearest to the end, and finally to the end.
    ...
    let start_nearest_intersection = this.agentmap.getNearestIntersection(
        start_street_point,
        start_place,
      ),
      goal_nearest_intersection = this.agentmap.getNearestIntersection(
        goal_street_point,
        goal_place,
      );

    start_street_feature =
      this.agentmap.streets.getLayer(start_street_id).feature;

    this.setTravelOnStreetNetwork(
      start_street_point,
      goal_street_point,
      start_nearest_intersection,
      goal_nearest_intersection,
      speed,
    );
  }
};
 ...
 * @param {number} speed - The speed that the agent should travel, in meters per tick.
 */
Agent.setTravelOnSameStreet = function (
  start_lat_lng,
  goal_lat_lng,
  street_feature,
  street_id,
  ...
  let start_coords = L.A.pointToCoordinateArray(start_lat_lng),
    goal_coords = L.A.pointToCoordinateArray(goal_lat_lng),
    street_path_unordered = L.A.reversedCoordinates(
      lineSlice(start_coords, goal_coords, street_feature).geometry.coordinates,
    );
  let start_to_path_beginning = start_lat_lng.distanceTo(
      L.latLng(street_path_unordered[0]),
    ),
    start_to_path_end = start_lat_lng.distanceTo(
      L.latLng(street_path_unordered[street_path_unordered.length - 1]),
    );
  let street_path =
    start_to_path_beginning < start_to_path_end
      ? street_path_unordered
      : street_path_unordered.reverse();
  ...
  if (street_path_lat_lngs.length > 1) {
    let second_lat = street_path_lat_lngs[1].lat,
      ...
      penultimate_lat =
        street_path_lat_lngs[street_path_lat_lngs.length - 2].lat,
      penultimate_lng =
        street_path_lat_lngs[street_path_lat_lngs.length - 2].lng;
 ...
 * @param {number} speed - The speed that the agent should travel, in meters per tick.
 */
Agent.setTravelOnStreetNetwork = function (
  start_lat_lng,
  goal_lat_lng,
  start_int_lat_lng,
  goal_int_lat_lng,
  ...
  let path = this.agentmap.getPath(
    start_int_lat_lng,
    goal_int_lat_lng,
    start_lat_lng,
    goal_lat_lng,
  ...
  for (let i = 0; i <= path.length - 2; i++) {
    let current_street_id = path[i].new_place.id,
      current_street_feature =
        this.agentmap.streets.getLayer(current_street_id).feature;
...
Agent.travel = function (override_speed) {
  let current_coords = L.A.pointToCoordinateArray(this.trip.current_point),
    ...
    sub_goal_coords = destination(
      current_coords,
      sub_goal_distance * 0.001,
      this.trip.angle,
    ).geometry.coordinates,
    sub_goal_lat_lng = L.latLng(L.A.reversedCoordinates(sub_goal_coords));

  let segment_to_goal = lineString(
      [this.trip.current_point, this.trip.goal_point].map((point) =>
        L.A.pointToCoordinateArray(point),
      ),
    ),
    segment_to_sub_goal = lineString(
      [this.trip.current_point, sub_goal_lat_lng].map((point) =>
        L.A.pointToCoordinateArray(point),
      ),
    );

  let goal_lat_dist = Math.abs(
      this.trip.current_point.lat - this.trip.goal_point.lat,
    ),
    goal_lng_dist = Math.abs(
      this.trip.current_point.lng - this.trip.goal_point.lng,
    );
  ...
  let sub_goal_lat_dist = Math.abs(
      sub_goal_lat_lng.lat - this.trip.current_point.lat,
    ),
    sub_goal_lng_dist = Math.abs(
      sub_goal_lat_lng.lng - this.trip.current_point.lng,
    );
  ...
  let half_meters = sub_goal_distance * 2,
    ...
    final_lat_step_value =
      this.trip.lat_dir *
      (sub_goal_lat_dist - Math.abs(int_lat_step_value * int_half_meters)),
    final_lng_step_value =
      this.trip.lng_dir *
      (sub_goal_lng_dist - Math.abs(int_lng_step_value * int_half_meters));
  ...
  for (let i = 0; i < int_half_meters; ++i) {
    ...
    if (this.trip.goal_point.move_directly === true) {
      let new_goal_lat_dist = Math.abs(
          this.trip.current_point.lat - this.trip.goal_point.lat,
        ),
        new_goal_lng_dist = Math.abs(
          this.trip.current_point.lng - this.trip.goal_point.lng,
        );

      if (
        new_goal_lat_dist > goal_lat_dist ||
        new_goal_lng_dist > goal_lng_dist
      ) {
...
Agent.step = function (lat_step_value, lng_step_value) {
  let new_lat_lng = L.latLng([
    this.trip.current_point.lat + lat_step_value,
    this.trip.current_point.lng + lng_step_value,
  ]);

  (this.trip.current_point = new_lat_lng), this.steps_made++;
  ...
  if (
    this.agentmap.animation_interval > 0 &&
    this.steps_made % this.agentmap.animation_interval === 0
  ) {
...
Agent.checkArrival = function (sub_goal_lat_lng, leftover_after_goal) {
  if (this.trip.goal_point.distanceTo(this.trip.current_point) < 0.1) {
    ...
    return true;
  } else if (sub_goal_lat_lng.distanceTo(this.trip.current_point) < 0.1) {
    this.trip.moving = false;
...
function seqUnitAgentMaker(id) {
  ...
  if (index > this.units.getLayers().length - 1) {
    throw new Error(
      "seqUnitAgentMaker cannot accommodate more agents than there are units.",
    );
  }
...
function agentify(count, agentFeatureMaker) {
  ...
  for (let i = agents_existing; i < agents_existing + count; i++) {
    ...
    let coordinates = L.A.reversedCoordinates(
        agent_feature.geometry.coordinates,
      ),
    ...
    if (!L.A.isPointCoordinates(coordinates)) {
      throw new Error(
        "Invalid feature returned from agentFeatureMaker: geometry.coordinates must be a 2-element array of numbers.",
      );
    } else if (typeof place.id !== "number") {
      throw new Error(
        "Invalid feature returned from agentFeatureMaker: properties.place must be a {unit: unit_id} or {street: street_id} with an existing layer's ID.",
      );
    }
