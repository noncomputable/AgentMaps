...
function reversedCoordinates(coordinates) {
  ...
  if (typeof coordinates[0] != "number") {
    for (let inner_coordinates of coordinates) {
      reversed.splice(
        reversed.indexOf(inner_coordinates),
        1,
        reversedCoordinates(inner_coordinates),
      );
    }
  } else {
...
function isPointCoordinates(array) {
  if (
    array.length !== 2 &&
    !(typeof array[0] == "number" && typeof array[1] == "number")
  ) {
    return false;
  }
...
function pointToCoordinateArray(point) {
  ...
  if (typeof point.lat === "number" && typeof point.lng === "number") {
    ...
    isPointCoordinates(point.geometry.coordinates)
  ) {
    ...
    throw new Error(
      "Invalid point: point must either be array of 2 coordinates, or an L.latLng.",
    );
  }
  ...
  for (let i = 0; i < arr_a.length; i++) {
    ...
    for (let j = 0; j < arr_b.length; j++) {
      ...
      if (isPointCoordinates(el_a) && isPointCoordinates(el_b)) {
        ...
        throw new Error(
          "Every element of each array must be a coordinate pair array.",
        );
      }
    }
  }
